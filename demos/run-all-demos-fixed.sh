...
check_server_health() {
    ...
    if [ "$connected" = false ]; then
        ...
        cd ../java-rest-api
        nohup java -jar target/gameauth-0.0.1-SNAPSHOT.jar server config.yml > ../demos/logs/server.log 2>&1 &
        cd ../demos
...
main() {
    ...
    demos=(
        ...
        "advanced/demo-intelligence-engine.sh|🧠 AI Player Intelligence Engine"
        "advanced/demo-forge-workshop.sh|🎮 Advanced Forge Workshop System"
        "advanced/demo-weapon-meta.sh|⚡ Weapon Balance & Meta Analysis"
        "advanced/demo-performance-stress.sh|🚀 High-Performance Load Testing"
        "advanced/demo-security-auth.sh|🔒 Advanced Security & Auth Demo"
        "advanced/demo-full-session.sh|🎯 Complete Game Session Simulation"
    )
